...
public class NumDataController {
    ...
    public Result queryNumDataBySensorId(@RequestBody DataConditionParam dataConditionParam, @CurrentUser Map tokenData) throws Exception {
        ...
        myVerificationUtil.verifySensorInUser(userId,dataConditionParam.getId());
        try {
    ...
    //@SystemControllerLog(logAction = "insertNumData", logContent = "新增多条数值数据，数组传值")
    ...
    public Result insertNumDatas(@RequestHeader String apiKey,@RequestParam int sensorId,@RequestParam(value = "values[]") double[] values) throws Exception {
        Result result = ResultUtil.success();
        myVerificationUtil.verifyApiKeyInUserLinkSensorId(apiKey,sensorId);
        try {
            //result = ResultUtil.success(numDataService.insertNumDatas(apiKey,sensorId,values));
        } catch (Exception e) {
            result = handle.exceptionGet(e);
        }
        return result;
    ...
    @SystemControllerLog(logAction = "deleteNumDatas", logContent = "批量删除数据点")
    @ApiOperation(value = "批量删除数据点", notes = "批量删除数据点")
    @ApiImplicitParams({
    ...
    public Result deleteNumDatas(@RequestParam(value = "ids[]") int[] ids,@RequestParam int sensorId, @CurrentUser Map tokenData) throws Exception {
        ...
        try {
            //result = ResultUtil.success(numDataService.deleteNumDatas(ids));
        } catch (Exception e) {
            result = handle.exceptionGet(e);
        }
        return result;
